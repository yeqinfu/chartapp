package com.ppandroid.app.utils.glide;
...
public abstract class GlideUtils {
   ...
   /* public static Target<GlideDrawable> loadImageFormVideo(@NonNull String url, @NonNull ImageView imageView) {
    ...
    public static void displayImage(Context context,String imageUrls,ImageView imageView){
        DebugLog.d("image----->"+imageUrls);
        imageUrls= GlideUtils.addImageBaseUrl(imageUrls);
        loadImage(context,imageUrls,imageView,null, R.drawable.pic_youling,R.drawable.pic_youling);
    }

    /**
     * 服务器有的图片有加http头，有的没有，很不规范，这边做一个容错处理
     * @return
     */
    public static String  addImageBaseUrl(String url){
        if (!TextUtils.isEmpty(url)&&!url.startsWith("http")){
            DebugLog.e("注意：检测该图片地址不是以http开头，添加默认图片首地址\n");
            url= Contants.Companion.getBaseUrl()+url;
            DebugLog.d("image----->"+url+"\n");
        }
        return url;
    }

    public static void displayImageHigh(Fragment fragment, String imageUrls, ImageView imageView){
        Glide
                .with(fragment)
                .load(imageUrls)
