...
import kotlinx.android.synthetic.main.fg_about_area.*
...
class FG_AboutArea : FG_Base(){
    ...
    override fun fgRes(): Int= R.layout.fg_about_area
            ...
            adapter?.let {
                if (it.checkPos==-1){
                    ...
                    message?.let {
                        ...
                        EventBus.getDefault().post(FG_AddDevices.ET_AddDevices(FG_AddDevices.ET_AddDevices.TASKID_ADD_DEVICES_ABOUT_AREA, chooseId, choooseName))
                        //关闭本页面和父级别页面
                        EventBus.getDefault().post(ET_SyStemSetting(ET_SyStemSetting.TASKID_CLOSE_FG_ABOUT_AREA))
                    }
    ...
    fun onMessageEvent(event: ET_SyStemSetting) {
        if (event.taskId == ET_SyStemSetting.TASKID_CLOSE_FG_ABOUT_AREA) {
           finish()
        }
    }
}