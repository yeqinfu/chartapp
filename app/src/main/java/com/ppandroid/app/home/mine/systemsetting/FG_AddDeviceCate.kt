...
import kotlinx.android.synthetic.main.fg_add_device_cate.*
 ...
 * 新建分项和修改分享
 */
class FG_AddDeviceCate :FG_Base(){
    ...
    override fun fgRes(): Int= R.layout.fg_add_device_cate
    ...
    override fun afterViews() {
        ...
        if (pageType == "1") {//修改仪表
            head_view.setCenterTitle("修改分项")
            if (parentId!="-1"){
                tv_parent_name.text=parentName
            }
            et_name.setText(objectName)
        } else {
            head_view.setCenterTitle("新建分项")
            if (parentId!="-1"){
                tv_parent_name.text=parentName
            }
        }
    ...
    private fun postInfo() {
        ...
        Http.post(activity,url,map,BaseBody::class.java,object: MyCallBack<BaseBody>{
            override fun onResponse(response: BaseBody?) {
                response?.let {
                    if (it.isSuccess) {
                        EventBus.getDefault().post(ET_SyStemSetting(ET_SyStemSetting.TASKID_REFRESH_DEVICE_CATE_PAGE))
                        if (pageType=="1"){
                            toast("修改成功")
                        }else{
                            toast("添加成功")
