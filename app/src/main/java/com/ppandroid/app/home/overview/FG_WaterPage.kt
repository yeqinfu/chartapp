/*
 * Created by yeqinfu on 17-12-8 下午3:59
...
import com.ppandroid.app.home.adapter.AD_AreaEnergyWater
import com.ppandroid.app.home.adapter.AD_EnergyWater
import com.ppandroid.app.home.adapter.AD_InstrumentWater
import com.ppandroid.app.home.adapter.AD_ZhongdianWater
...
import kotlinx.android.synthetic.main.fg_water_page.*
 ...
 */
class FG_WaterPage:FG_Base(){
    override fun fgRes(): Int = R.layout.fg_water_page
    ...
    fun onMessageEvent(event: FG_OverView.ET_OverView) {
        if (event.taskId === FG_OverView.ET_OverView.TASKID_REFRESH_WATER) {
            loadOverViewConfig()
        }
    ...
    private var body: BN_OverViewWater? = null

    private fun loadContent() {
        var url="user/overview/water/index.json"
    ...
    private fun addEnergyInfo(): View {
        ...
        view.find<TextView>(R.id.tv_used_month).text="本月用水"
        view.find<TextView>(R.id.tv_use_water_title).text="用水量"
    ...
    private fun addModule(): View {
        ...
        iv_add_model.setOnClickListener {
            var b=FG_OverViewConfig.createIntent(1)
            startAC(FG_OverViewConfig::class.java.name,b)
        }
    ...
    private fun setDeviceInformationValue(view: View) {
        ...
        zhongdianBody?.let {
            /**添加仪表用能统计图*/
            var adapter = AD_ZhongdianWater(activity, it)
            var view_pager_zhongdian = view.find<ViewPager>(R.id.view_pager_zhongdian)
            adapter.lisenter=object : FG_OverView.IHeightListener {
    ...
    private fun setInstrumentInfomationValue(view: View){
        ...
        instrumentBody?.let {
            /**添加仪表用能统计图*/
            var adapter = AD_InstrumentWater(activity, it)
    ...
    private fun setEnergyValue(view:View) {
        ...
        energyBody?.let {
            ...
            var adapter = AD_EnergyWater(activity, it.classificationInformationList)
            ...
            view_pager.setOnPageChangeListener(object : ViewPager.OnPageChangeListener {
                ...
                override fun onPageSelected(position: Int) {
                    ...
                    setEnergyInfo(view, it.classificationInformationList[position].classificationM3MapList)
                }

            })
            setEnergyInfo(view, it.classificationInformationList[0].classificationM3MapList)

            /**区域用电统计*/
            var view_pager_energy = view.find<ViewPager>(R.id.view_pager_energy)
            view_pager_energy.adapter = AD_AreaEnergyWater(activity, it.areaInformationList)
    ...
    private fun setEnergyInfo(view: View, list: List<BN_OverViewWater.MessageBean.OverviewConsumptionInformationBean.ClassificationInformationListBean.ClassificationM3MapListBean>) {

        if (TextUtils.isEmpty(select)){
            view.find<TextView>(R.id.tv_energy_title).text="今日用水"
        }else{
            view.find<TextView>(R.id.tv_energy_title).text=select+"用水"
        }
        ...
        if (list.isNotEmpty()){
            ...
            toCateAnalysisPage(ll_01,list[0].id?.toString(),list[0].key)
        }else{
        ...
        if (list.size>=2){
            ...
            toCateAnalysisPage(ll_02,list[1].id?.toString(),list[1].key)
        }else{
        ...
        if (list.size>=3){
            ...
            toCateAnalysisPage(ll_03,list[2].id?.toString(),list[2].key)
        }else{
        ...
        if (list.size>=4){
            ...
            toCateAnalysisPage(ll_04,list[3].id?.toString(),list[3].key)
        }else{
        ...
        if (list.size>=5){
            ...
            toCateAnalysisPage(ll_05,list[4].id?.toString(),list[4]?.key)
        }else{
