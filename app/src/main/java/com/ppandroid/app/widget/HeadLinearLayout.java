package com.ppandroid.app.widget;

import android.content.Context;
import android.support.annotation.Nullable;
import android.util.AttributeSet;
import android.widget.LinearLayout;

/**
 * Created by yeqinfu on 2017/8/23.
 */

public class HeadLinearLayout extends LinearLayout{
    public HeadLinearLayout(Context context) {
        super(context);
    }

    public HeadLinearLayout(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }

    public HeadLinearLayout(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }


}
